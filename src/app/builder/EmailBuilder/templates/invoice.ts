...
import { IEmailTemplate, IEmailTheme, IEmailComponent } from '../EmailBuilder';
...
export const invoice: IEmailTemplate = {
  ...
  render: (variables, theme, components) => {
    ...
    const tableRows = items.map((item: InvoiceItem) => [
      item.name,
    ...
    if (subtotal) totalRows.push(['Subtotal', '', subtotal]);
    if (discount) totalRows.push(['Discount', '', `-${discount}`]);
    if (tax) totalRows.push(['Tax', '', tax]);
